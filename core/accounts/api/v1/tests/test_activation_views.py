...
def common_user():
    return {
        "user": CostumeUser.objects.create_user(
            email="test@gmail.com", password="Aaa123##"
        ),
    }
...
class TestActivation:
    ...
    def test_get_response_200(
        self, api_client
    ):  # token is correct (user verified successfully)
        user = common_user()
        token = get_token_for_user(user["user"])
        url = reverse(
            "accounts:api_v1:user:activation", kwargs={"token": token}
        )
        response = api_client.get(path=url)
        assert response.status_code == 200
        assert CostumeUser.objects.get(id=user["user"].id).is_verify is True

    def test_get_response_400(
        self, api_client
    ):  # token is incorrect (user verified failed)
        user = common_user()
        url = reverse(
            "accounts:api_v1:user:activation", kwargs={"token": "123456789"}
        )
        response = api_client.get(path=url)
...
class TestActivationResend:
    ...
    def test_post_response_200(
        self, api_client
    ):  # token resend successfully for available user in db
        user = common_user()
    ...
    def test_post_response_404(
        self, api_client
    ):  # token resend failed ( unavailable user in db )
        data = {"email": "unavailable@user.com"}
        url = reverse("accounts:api_v1:user:activation-resend")
        response = api_client.post(path=url, data=data)
