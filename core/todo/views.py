...
class TodoView(View):
    def get(self, request):
        try:
            tasks_done = TodoModel.objects.filter(
                profile__user=request.user, is_done=True
            ).order_by("-updated")
            tasks_active = TodoModel.objects.filter(
                profile__user=request.user, is_done=False
            ).order_by("dead_end")
        except TypeError:
            tasks_done = {}
            tasks_active = {}
...
class AddDayView(View):
    def get(self, request, day: int):
        if 1 <= day <= 31:
            ...
            messages.warning(
                request, "day number should be between 1 and 31", "red-600"
            )
...
class AddMonthView(View):
    def get(self, request, month: int):
        if 1 <= month <= 12:
            ...
            messages.warning(
                request, "month number should be between 1 and 12"
            )
        return redirect("todo:main_page")
...
class AddLevelView(View):
    def get(self, request, level: int):
        if 1 <= level <= 3:
            ...
            messages.warning(
                request, "level number should be between 1 and 3"
            )
        return redirect("todo:main_page")
...
class ClearCompletedTaskView(View):
    ...
    def get(self, request):
        tasks = TodoModel.objects.filter(
            profile_id=profile_information(request)["profile"].id,
            is_done=True,
        )
        tasks.delete()
